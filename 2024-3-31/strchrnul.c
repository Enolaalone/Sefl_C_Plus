...
char *
__strchrnul (const char *str, int c_in)//返回c_in第一出现的地址
{
  /* Align pointer to sizeof op_t.  */
  uintptr_t s_int = (uintptr_t) str;//地址转换uintptr_t类型操作
  // 在64位的机器上，intptr_t和uintptr_t分别是long int、unsigned long int的别名；在32位的机器上，intptr_t和uintptr_t分别是int、unsigned int的别名。
  const op_t *word_ptr = (const op_t *) PTR_ALIGN_DOWN (str, sizeof (op_t));//按op_t大小向前对齐
