...
void Find(Node *nodes,FILE *pf,FILE *fp,int n,char *letter){
    int q=n-1;
    while(fread(letter,1,1,fp)>=1)
    {
        for (int i =7; i >=0; i--)
        {
            if(nodes[q].l_kid==-1 || nodes[q].r_kid==-1){
                fprintf(pf,"%s",nodes[q].name);
                // printf("%d",q);
                q=n-1;
...
int main(){
    ...
    // FILE*pff=fopen("k.txt","w");

    // for (int i = 0; i < 2*(*n)-1; i++)
    // {
    //     fprintf(pff,"%s\t%d\t%d\n",nodes[i].name,nodes[i].l_kid,nodes[i].r_kid);
    // }
    // fclose(pff);
    // stack_code(nodes,*n);

    FILE *fp=fopen("bin.txt","rb");
    FILE *pf=fopen("SG.txt","w");
    char letter=0;
    Find(nodes,pf,fp,(*n)*2-1,&letter);
