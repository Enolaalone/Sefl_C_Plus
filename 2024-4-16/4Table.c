        ...
        if(!g_num)break;
            ...
            while(1){
                ...
                if(!num)break;
                    ...
                    char *e=(char*) malloc(sizeof (char));
                    printf("输入需要查询的顶点,以及邻接顶点位序\n");
                    scanf("%c %d",e,&locate);

                    TF= NextAdjVex(G,*e,locate);
                    if(TF!=False&& TF!=OVERFLOW){
                        printf("%c顶点的第%d邻接顶点位置序号为%d\n",*e,locate,TF);
                    }
                    free(e);
                }
                    ...
                    printf("输入需要插入的边的起点和终点\n");
                    scanf("%c %c",e,e+1);

                    if(InsertArc(&G,e[0],e[1])){
                        printf("边%c-%c插入成功！\n",e[0],e[1]);
                    }
                    free(e);
                }
                    ...
                    char *e=(char*) malloc(2*sizeof (char));
                    printf("输入需要删除的边的起点和终点\n");
                    scanf("%c %c",e,e+1);

                    if(DeleteArc(&G,e[0],e[1])){
                        printf("边%c-%c删除成功！\n",e[0],e[1]);
                    }
                    free(e);
                }
