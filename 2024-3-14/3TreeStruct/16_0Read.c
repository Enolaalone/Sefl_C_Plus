...
int main() {
    ...
    char*definition=(char*) malloc(100*sizeof (char)),value,e,E[5],name[20];
    int i,LR;
    for (int j = 0; j < 100; ++j) {
        M[j]=NULL;
    }
            ...
            if(t_num>99 || t_num<0){
                ...
                while(1){
                    ...
                    if(!num)break;
                        ...
                        if(M[t_num]){
                            int k=0;
                            definition=(char*) malloc(1000*sizeof (char));
                            printf("输入文件名称!\n");
                            scanf("%s",name);
                            FILE *fp= fopen(name,"r");
                            while(fread(&definition[k],sizeof (char ),1,fp)){ k++;printf("%c ",definition[k-1]);}
                            definition[k]='\0';
                            if(read(&M[t_num]->l_kid,&definition)){
                                printf("文件读取成功!\n");
                            } else{
                                printf("文件读取失败!/n");
                            }
...
bool DeleteNode(Tree *T,char e){
    if(T->l_kid!=NULL&&T->l_kid->key==e)Delecel(T);
    else if(T->r_kid!=NULL&&T->r_kid->key==e)Delecer(T);
    else{
        if(T->l_kid)DeleteNode(T->l_kid,e);//遍历左节点
        if(T->r_kid)DeleteNode(T->r_kid,e);//遍历右节点
    }
    return true;
...
bool LevelOrderTraverse(Tree *T){
    Tree *ptrTree[1000];//指针数组寄存出入
    int in=0,out=0;
    ptrTree[in++]=T;//初始

    while (in>out){
        if(ptrTree[out]){
            printf("%c %c\n",ptrTree[out]->key,ptrTree[out]->data);
            ptrTree[in++]=ptrTree[out]->l_kid;
            ptrTree[in++]=ptrTree[out]->r_kid;
        }
        out++;
    }return true;
}//按层遍历
...
bool TreeWrite(Tree *T,FILE *fp){
    ...
    if(!T){
        ...
        fprintf(fp,"%c%c",T->key,T->data);//写入数据
