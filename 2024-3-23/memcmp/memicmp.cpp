...
extern "C" int __cdecl _memicmp_l (//使用C标准
        void const * const lhs,//指针本身是常量 指向的值也是常量 都不可改变
...
{
    /* validation section */
    _VALIDATE_RETURN(lhs != nullptr || count == 0, EINVAL, _NLSCMPERROR);//false:空指针 但count不为0 执行该语句
    _VALIDATE_RETURN(rhs != nullptr || count == 0, EINVAL, _NLSCMPERROR);

    if (count == 0)//如果count为0结束,比较前0个字符就结束
    {
        return 0;
    }

    unsigned char const * lhs_ptr = static_cast<unsigned char const *>(lhs);//强制转换为unsigned char const *
    ...
    do
    {
        result = _tolower_fast_internal(*lhs_ptr++, _loc_update.GetLocaleT())//GetLocale 函数返回当前的 locale ID
