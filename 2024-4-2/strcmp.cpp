...
{
  unsigned int idx = index_first_zero_ne (w1, w2);//不同出现的位置
  return extractbyte (w1, idx) - extractbyte (w2, idx);//应该是摘录了不同的字节，做差比较
}
///------------------------------------------------------------------------------
...
strcmp (const char *p1, const char *p2)
{
  ...
  /* P1 is now aligned to op_t.  P2 may or may not be. p1对齐了op_t p2没有 */
  const op_t *x1 = (const op_t *) p1;//x1=p1
  op_t w1 = *x1++;
  uintptr_t ofs = (uintptr_t) p2 % sizeof(op_t);//p2向前对齐unsigned int
  return ofs == 0
    ? strcmp_aligned_loop (x1, (const op_t *)p2, w1)//p2也对齐了op_t；x1++了，w1保存了++之前的内容
